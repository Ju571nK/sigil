...
pub async fn get_fleet_compliance(
    ...
    let server_version: Option<i64> = std::fs::read(&state.policy_bundle_path).ok()
    ...
    let start = match &q.cursor {
        None => 0,
        Some(c) => hosts.iter().position(|h| &h.host_id == c).map(|i| i + 1).unwrap_or(hosts.len()),
    };
    let end = (start + limit).min(hosts.len());

    let rows: Vec<Value> = hosts[start..end].iter().map(|h| {
    ...
    let next_cursor = if end < hosts.len() {
        hosts.get(end - 1).map(|h| Value::String(h.host_id.clone())).unwrap_or(Value::Null)
    } else { Value::Null };
