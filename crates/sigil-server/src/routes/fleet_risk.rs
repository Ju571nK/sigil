...
fn clamp_limit(req: Option<u32>) -> usize { req.unwrap_or(100).clamp(1, 1000) as usize }
...
pub async fn get_fleet_risk(
    ...
    let tool_filter: Option<Vec<String>> = q.tool.as_ref()
        .map(|v| v.split(',').map(|s| s.trim().to_string()).collect());

    // Collect (host, top_tool, top_entry) for each host that has any risk meeting filters.
    let mut rows: Vec<(HostSummary, sigil_core::event::AiTool, crate::fleet_index::RiskEntry)> = Vec::new();
    for h in state.fleet_index.snapshot_all() {
        ...
        for (tool, entry) in &h.current_risk {
            if let Some(tf) = &tool_filter {
                let tstr = serde_json::to_value(tool).unwrap().as_str().unwrap().to_string();
                if !tf.iter().any(|t| t == &tstr) { continue; }
            }
            if bucket_rank(entry.bucket) < min { continue; }
            let pick = top.as_ref().map(|(_, e)| bucket_rank(entry.bucket) > bucket_rank(e.bucket)).unwrap_or(true);
            if pick { top = Some((*tool, entry.clone())); }
        }
        if let Some((tool, entry)) = top {
            rows.push((h, tool, entry));
        }
    }
    rows.sort_by(|a, b| bucket_rank(b.2.bucket).cmp(&bucket_rank(a.2.bucket))
        .then_with(|| b.2.score.partial_cmp(&a.2.score).unwrap_or(std::cmp::Ordering::Equal)));

    let start = match &q.cursor {
        None => 0,
        Some(c) => rows.iter().position(|(h, _, _)| &h.host_id == c).map(|i| i + 1).unwrap_or(rows.len()),
    };
    ...
    let next_cursor = if end < rows.len() {
        rows.get(end - 1).map(|(h, _, _)| Value::String(h.host_id.clone())).unwrap_or(Value::Null)
    } else { Value::Null };
